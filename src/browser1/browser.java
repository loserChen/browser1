...
public class browser {
	...
	private String homePage = "www.zjut.edu.cn";           //主页
	...
	private Browser browser;                              //浏览器
	private Composite composite;
	private Button btnBack;                               //后退按钮
	private GridData gd_btnBack;                         
	private Button btnForword;                             //前进
	...
	java.util.List collection = new ArrayList();              // 收藏夹
	private TabItem tabItem_1;                               //标签
	...
	private void initMenu(){
		...
		mntmHistoryDeletion.setText("清除历史记录");
		for(int i=0;i<5;i++)
		{
			array[i]="";
		}
	...
	private void initBrowser(){//浏览器的初始化
...
//源代码按钮
	...
	private void initStatus(){//状态栏的初始化
	...
	private void method(){//具体操作全部封装在一起
		...
		public void changed(StatusTextEvent e) {
		    if (loadCompleted == false) {
		     lblStatus.setText(e.text);               //在底部状态栏显示链接信息
		    } 	
		   }
		  });
		browser.addOpenWindowListener(new OpenWindowListener() {// 在当前页面中打开点击的链接页面
		    ...
		    e.display.asyncExec(new Runnable(){                //asyncExec能直接返回，不等待runnable中代码被执行完
		       public void run() {
		        method(); 
		       }
		      });
			...
			public void widgetSelected(SelectionEvent arg0){
				...
				if(url!=null)
				{
					...
					try 
					{
						...
						//调用函数实现点击IP栏下拉框能显示历史记录并点击能实现跳转的功能
...
//获取网页源代码
	...
	//新建标签项
	 ...
	 private void addUrlToCombo(String url){  //判断url是否已经被存储
	 ...
	 private void addIpToCombo(String ip){//判断IP是否已经被存储
	...
	//获取源代码
		public void getSource (String url) 
		{
			try 
			{
				...
				linesep=System.getProperty("line.separator"); //换行符,功能和"\n"是一致的,但是此种写法屏蔽了Windows和Linux的区别
				Source="";
				//根据URL得到的源代码
